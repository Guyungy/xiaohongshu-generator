...
## 通用创作规则 (Universal Creation Rules) - 第一优先级

本 skill 的所有内容生成请求，无论使用何种 persona，都必须遵循以下创作核心原则：
*   **深度利他**: 产出的内容必须具有极强的利他属性，提供真实、高价值、解决实际问题的洞察和建议。避免空泛说教，专注于为读者创造实际价值。
*   **信息密度与深度**: 内容应具备足够的文字量和信息深度，能够对主题进行全面、透彻的分析，而非流于表面。

人设内部的创作规则作为对通用规则的补充和细化，但不得与通用规则相悖。
